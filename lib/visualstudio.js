...
exports.detect = function detect(options, callback) {
	...
	async.series([
		...
		async.parallel(Object.keys(possibleVersions).map(function (configKey) {
			...
			results.visualstudio[preferredVersion && results.visualstudio[preferredVersion] ? preferredVersion : Object.keys(results.visualstudio).sort().pop()].selected = true;

			next(null, results.visualstudio);
		});
	});
